...
import { applyTheme, getStoredTheme, getSystemPref, toggleTheme } from "../utils/theme";
...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    setTheme(nextTheme);
    applyTheme(nextTheme, { persist: Boolean(stored) });
  }, []);
  ...
  return (
    <footer className="mt-24 border-t border-gray-200 bg-white/90 backdrop-blur dark:border-white/10 dark:bg-[#0B1220]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
          ...
          <nav aria-label="Social media" className="flex items-center gap-3">
            {socials.map((social) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-full border border-gray-200 p-2 text-[#1C3B71] transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#1F4086] focus:ring-offset-2 focus:ring-offset-white dark:border-white/10 dark:text-white dark:hover:bg-white/10 dark:focus:ring-offset-[#0B1220]"
        ...
        <p className="mt-8 max-w-3xl text-sm text-gray-600 dark:text-white/70">
